#!/usr/bin/env python3
"""
GPU Pricing Fetcher v7 — Neysa Competitive Intelligence

New vs v6:
- AWS SageMaker GPU instances added (ml.p5, ml.g6, ml.g6e series)
  Same GPUs as EC2 but with managed platform overhead (~10-30% more expensive)

Providers: Azure, OCI, AWS EC2, AWS SageMaker, GCP, RunPod, Vultr, Vast.ai
"""
    ...
    for region in REGIONS["azure"]:
        for arm_sku, (gpu, count) in AZURE_SKUS.items():
            ...
            try:
                ...
                if (arm_sku, region, "on_demand") not in seen:
                    seen.add((arm_sku, region, "on_demand"))
                ...
                for sp in item.get("savingsPlan", []):
                    ...
                    if not sp_price:
                        continue
                    tier = "12mo" if "1 Year" in term else "36mo" if "3 Year" in term else None
                    if tier and (arm_sku, region, tier) not in seen:
                        seen.add((arm_sku, region, tier))
                        skus.append({"sku": arm_sku, "gpu_type": gpu, "gpu_count": count,
...
# ─── PROVIDER 4: AWS SAGEMAKER ───────────────────────────────────────────────
# Same GPU hardware as EC2 but with SageMaker managed platform overhead.
# ml. prefix = SageMaker managed instance. Typically 10-30% more than EC2.
SAGEMAKER_INSTANCES = {
    "ml.p5.48xlarge":     ("H100 SXM",  8),
    "ml.p5e.48xlarge":    ("H200 SXM",  8),
    "ml.p5en.48xlarge":   ("H200 SXM",  8),
    "ml.g6.xlarge":       ("L4",        1),
    "ml.g6.4xlarge":      ("L4",        1),
    "ml.g6.12xlarge":     ("L4",        4),
    "ml.g6.48xlarge":     ("L4",        8),
    "ml.g6e.xlarge":      ("L40S",      1),
    "ml.g6e.4xlarge":     ("L40S",      1),
...
def _fetch_aws_generic(client, instances, service_code, instance_field, regions):
    """Shared fetch logic for EC2 and SageMaker."""
    skus = []
    for region in regions:
        for inst, (gpu, count) in instances.items():
            filters = [
                {"Type": "TERM_MATCH", "Field": instance_field, "Value": inst},
                ...
                for term in obj.get("terms", {}).get("OnDemand", {}).values():
                    for dim in term.get("priceDimensions", {}).values():
                        ...
                        if price > 0:
                            ...
                            break
                    break
                # Reserved (EC2 only — SageMaker has limited RI options)
                if service_code == "AmazonEC2":
                    for term_data in obj.get("terms", {}).get("Reserved", {}).values():
                        attrs = term_data.get("termAttributes", {})
                        if attrs.get("OfferingClass") != "standard" or \
                           attrs.get("PurchaseOption") != "No Upfront":
                            continue
                        lease = attrs.get("LeaseContractLength", "")
                        tier  = {"1yr": "12mo", "3yr": "36mo"}.get(lease)
                        if not tier:
                            continue
...
def fetch_aws():
    try:
        ...
        return {"provider": "AWS EC2", "error": "boto3 not installed", "skus": []}
    try:
        client = boto3.client("pricing", region_name="us-east-1")
    except Exception as e:
        return {"provider": "AWS EC2", "error": str(e), "skus": []}
    try:
        skus = _fetch_aws_generic(client, AWS_EC2_INSTANCES, "AmazonEC2",
                                  "instanceType", REGIONS["aws"])
        return {"provider": "AWS EC2", "skus": skus}
    except Exception as e:
        if "NoCredentials" in str(e):
            return {"provider": "AWS EC2", "error": "No AWS credentials", "skus": []}
        return {"provider": "AWS EC2", "error": str(e), "skus": []}


def fetch_sagemaker():
    """AWS SageMaker — same GPUs as EC2, higher price due to managed platform."""
    try:
        import boto3
        from botocore.exceptions import NoCredentialsError
    except ImportError:
        return {"provider": "AWS SageMaker", "error": "boto3 not installed", "skus": []}
    try:
        client = boto3.client("pricing", region_name="us-east-1")
    except Exception as e:
        return {"provider": "AWS SageMaker", "error": str(e), "skus": []}

    # SageMaker Pricing API uses "instanceName" not "instanceType"
    # and ServiceCode "AmazonSageMaker"
    # We try "instanceName" first; if empty results, also try "instanceType"
    skus = []
    for region in REGIONS["aws"]:
        for inst, (gpu, count) in SAGEMAKER_INSTANCES.items():
            price = None
            for field_name in ["instanceName", "instanceType"]:
                for component in ["Hosting", "Training", ""]:
                    filters = [
                        {"Type": "TERM_MATCH", "Field": field_name,   "Value": inst},
                        {"Type": "TERM_MATCH", "Field": "regionCode", "Value": region},
                    ]
                    if component:
                        filters.append({"Type": "TERM_MATCH", "Field": "component",
                                        "Value": component})
                    try:
                        response = client.get_products(ServiceCode="AmazonSageMaker",
                                                       Filters=filters, MaxResults=5)
                        for ps in response.get("PriceList", []):
                            obj = json.loads(ps)
                            for term in obj.get("terms", {}).get("OnDemand", {}).values():
                                for dim in term.get("priceDimensions", {}).values():
    ...
    return {"provider": "AWS SageMaker", "skus": skus}
...
def fetch_vultr():
    ...
    while True:
        try:
            ...
            resp = requests.get("https://api.vultr.com/v2/plans",
        ...
        for plan in data.get("plans", []):
            ...
            skus.append({"plan_id": plan.get("id"), "gpu_type": gpu_type,
                         "gpu_type_raw": plan.get("gpu_type"), "gpu_count": gpu_count,
                         "region": ",".join((plan.get("locations") or [])[:5]),
                         "price_tier": "on_demand",
...
def fetch_vastai():
    ...
    buckets  = {}
    for search_term, our_gpu in VASTAI_GPU_QUERIES:
        try:
            params = {"q": json.dumps({"rentable": {"eq": True}, "verified": {"eq": True},
            ...
            for offer in resp.json().get("offers", []):
                ...
                if dph > 0:
                    ...
                    buckets.setdefault(our_gpu, []).append(per_gpu)
        except Exception:
            pass
    skus = []
    for gpu_type, prices in buckets.items():
        if prices:
            prices.sort()
            skus.append({"gpu_type": gpu_type, "region": "global", "price_tier": "spot",
                         "per_gpu_hourly_usd": round(statistics.median(prices), 4),
                         "note": f"marketplace median ({len(prices)} offers)"})
...
PROVIDERS_ORDER = ["Azure", "AWS EC2", "AWS SageMaker", "GCP", "OCI",
                   "RunPod", "Vultr", "Vast.ai"]
TIER_LABELS     = {
...
def write_comparison_csv(output, path):
    ...
    comp_cols  = [f"{p.lower().replace(' ', '_').replace('.', '')}_{t}"
                  for p in PROVIDERS_ORDER for t in COMP_TIERS]
    guide = {
        "gpu_type": "COLUMN GUIDE",
        "pricing_note": "India pricing used where available (IN); US fallback (US)",
        **{f"neysa_{t}": f"Neysa — {TIER_LABELS[t]}" for t in NEYSA_TIERS},
        **{f"{p.lower().replace(' ', '_').replace('.', '')}_{t}": f"{p} — {TIER_LABELS[t]}"
           for p in PROVIDERS_ORDER for t in COMP_TIERS},
    }
    rows = []
    for gpu in GPU_TYPES:
        ...
        for pname in PROVIDERS_ORDER:
            ...
            india = INDIA_REGIONS.get(pname.split()[0].lower(), "")
            us    = US_REGIONS.get(pname.split()[0].lower(), "")
            key   = pname.lower().replace(" ", "_").replace(".", "")
            for t in COMP_TIERS:
                price, region = _best_price(skus, gpu, t, india, us)
                col = f"{key}_{t}"
                if price:
                    tag = (" (IN)" if region == india else " (US)" if region == us
                           else " (global)" if region == "global" else f" ({region})")
...
def print_summary(output):
    ...
    for k, v in output["providers"].items():
        ...
        if v.get("error"):
            print(f"  ✗ {name:20s}  {v['error'][:50]}")
        else:
            print(f"  ✓ {name:20s}  {len(v.get('skus', []))} SKUs")
    print()
